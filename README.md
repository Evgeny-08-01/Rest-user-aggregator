...
- Бэкап создаётся **автоматически** при запуске сервера (если есть миграции)
- Бэкап создаётся **перед `migrate-down`** (с подтверждением)
- Хранится в `./backups/` в сжатом формате `.sql.gz`
- Ротация: хранит последние **7** бэкапов (настраивается через `BACKUP_KEEP`)

### Команды
```bash
make backup          # Создать бэкап вручную (с подтверждением)
make restore         # Восстановить БД из выбранного бэкапа
make migrate-down    # Откатить миграции (с бэкапом)
```
#### Настройка
```bash
env
BACKUP_KEEP=7        # Количество хранимых бэкапов
```
#### Восстановление
```bash
make restore
Выбери файл из списка и подтверди
